...
// --- NY SIDA: INTEGRITETSPOLICY FÖR GOOGLE PLAY ---
app.get('/privacy', (req, res) => {
  res.send(`
    <html>
      <head><title>Integritetspolicy - Budget kollen</title><meta charset="utf-8"></head>
      <body style="font-family:sans-serif; padding:20px; max-width:800px; margin:0 auto; line-height:1.6;">
        <h1>Integritetspolicy för Budget kollen</h1>
        <p><strong>Senast uppdaterad:</strong> 2026-01-21</p>
        <p>Budget kollen värnar om din personliga integritet. Denna policy förklarar vilken data vi samlar in, varför vi gör det och hur den används.</p>
        <h3>1. Data vi samlar in</h3>
        <p>För att tillhandahålla tjänsten samlar vi in följande information:</p>
        <ul>
          <li><strong>E-postadress:</strong> Används för att skapa ditt konto, säkerställa inloggning och (om du aktivt valt det) skicka notiser och veckosummeringar.</li>
          <li><strong>Transaktionsdata:</strong> Information om inkomster, utgifter och budgetmål som du själv registrerar i appen.</li>
        </ul>
        <h3>2. Hur vi använder din data</h3>
        <p>All data lagras krypterat i en säker databas. Vi använder enbart datan för att ge dig tillgång till din egen budgethistorik och statistik.</p>
        <ul>
            <li>Vi säljer <strong>aldrig</strong> din data till tredje part.</li>
            <li>Vi delar <strong>aldrig</strong> din data med annonsörer.</li>
        </ul>
        <h3>3. Radera data</h3>
        <p>Du har rätt att när som helst begära att få all din data raderad.</p>
        <p>Kontakta oss på din registrerade e-postadress med ämnesraden "Radera konto", så tar vi bort ditt konto och all historik permanent.</p>
        <h3>4. Kontakt</h3>
        <p>Vid frågor, kontakta utvecklaren.</p>
      </body>
    </html>
  `);
});
  ...
  try {
    ...
    if (!user) {
      ...
      if (email && process.env.BREVO_API_KEY) {
        const emailHtml = `<div style="font-family: sans-serif; padding: 20px; color: #333;"><h2 style="color: #0084ff;">Välkommen till Budget kollen! 🚀</h2><p>Kul att du är igång. Här är dina inloggningsuppgifter:</p><div style="background: #f0f2f5; padding: 15px; border-radius: 10px; display: inline-block;"><p style="margin: 5px 0;">👤 Användarnamn: <b>${username}</b></p><p style="margin: 5px 0;">🔑 Lösenord: <b>${password}</b></p></div><p>Spara detta mejl eller byt lösenord om du vill vara extra säker.</p><p>Lycka till med sparandet!<br>/ Budget kollen</p></div>`;
...
app.post("/api/spend", authenticateToken, async (req, res) => { const user = await User.findById(req.user.id); const amount = Number(req.body.amount); if (req.body.isIncome) user.remainingBudget += amount; else user.remainingBudget -= amount; user.transactions.push(req.body); if (user.totalSavings >= 5000 && !user.milestones.includes("Spar-kung")) user.milestones.push("Spar-kung"); await user.save(); res.json({ success: true }); });

...
app.get("/api/overview", authenticateToken, async (req, res) => {
  ...
  // FIX: Sätt klockslag till midnatt för exakt beräkning
  const now = new Date();
  ...
  // Uppdatera streak men ändra inte lastActive förrän vi jämfört
  if (lastActiveStr !== todayStr) {
     const yesterday = new Date(now); 
     yesterday.setDate(yesterday.getDate() - 1);
     // Jämför strängar för att undvika tids-strul
     if (user.lastActive && user.lastActive.toDateString() === yesterday.toDateString()) {
       user.streak = (user.streak || 0) + 1;
     } else {
       user.streak = 1;
     }
     user.lastActive = new Date(); // Spara tidpunkt för inlogg
     await user.save();
  }

  // FIX: Lönedagsberäkning (Midnatt till Midnatt)
  ...
  res.json({ 
    ...
    daysLeft, // Skickar nu exakt antal dagar
